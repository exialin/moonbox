...
package moonbox.core.datasys.postgresql
...
import org.apache.spark.sql.sqlbuilder.{MbPostgreSQLDialect, MbSqlBuilder}
import org.apache.spark.sql.types._

import scala.collection.mutable.ArrayBuffer

class PostgresqlDataSystem(props: Map[String, String])
  ...
  override def fastEquals(other: DataSystem): Boolean = {
    other match {
      case postgresql: PostgresqlDataSystem =>
        socket == postgresql.socket
      case _ => false
    }
  }
  ...
  private def socket: (String, Int) = {
    ...
    val removeProtocol = url.stripPrefix("jdbc:postgresql://")
  ...
  override def buildScan(plan: LogicalPlan, sparkSession: SparkSession): DataFrame = {
    val sqlBuilder = new MbSqlBuilder(plan, new MbPostgreSQLDialect)
  ...
  override def buildQuery(plan: LogicalPlan, sparkSession: SparkSession): DataTable = {
    ...
    val resultSet = try {
      ...
      case e: Exception =>
        conn.close()
        logWarning("postgresql pushdown execute failed.", e)
        throw e
    }
  ...
  override def tableNames(): Seq[String] = {
    val schema = props.getOrElse("schema", "public")
  ...
  override def test(): Unit = {
    ...
    try  {
      ...
      case e: Exception =>
        logError("postgresql test failed.", e)
        throw e
    } finally {
      if (connection != null) {
        connection.close()
